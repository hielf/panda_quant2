...
n = close_desceding_x
m = close_desceding_rate_x
p = amount_desceding_x
q = amount_rising_count_bp
c = close_rising_count_s
d = close_rising_rate_s
market_df = df.copy()
id = 1
...
while market_df.shape[0] > 1:
    ...
    if flag == True:
        ...
        if s_p_id > b_p_id:
            ...
            # print (mask_df)
            # print (points)
            fig, _ = plottingfunction(mask_df, points)
            fig.savefig("./plts/" + stock_code + "_" + str(id) + ".png")
            net = net + (s_p - b_p)
            print ("profit:", net)
